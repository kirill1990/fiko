...
public class WinMain extends JFrame {
	...
	jSearchTextField.getDocument().addDocumentListener(
		new SearchDocumentListener());
	...
	jDataTable.addMouseListener(new MouseAdapter() {
		...
		if (e.getClickCount() == 2) {
		    ...
		    new WinReview(jDataTable.getValueAt(
	...
	jDataTable.addMouseListener(new MouseAdapter() {
		...
		if (0 < jDataTable.getSelectedRows().length && Me.isMetaDown()) {
			...
			JMenuItem svod = new JMenuItem("Создать свод за "
				+ jDataTable.getValueAt(
					jDataTable.getSelectedRows()[0], 2)
				+ " "
				+ jDataTable.getValueAt(
					jDataTable.getSelectedRows()[0], 1));
			Pmenu.add(svod);

			//
			svod.addActionListener(new ActionListener() {
			    public void actionPerformed(ActionEvent e) {
				String month = jDataTable.getValueAt(
					jDataTable.getSelectedRows()[0], 2)
					.toString();
				String year = jDataTable.getValueAt(
					jDataTable.getSelectedRows()[0], 1)
					.toString();
				...
				for (int p = 0; p < districts.length; p++) {
				    Vector<Integer> ids = new ConnectionBD()
					    .getIDS(year, month, districts[p]);

				    if (ids.size() > 0) {
					full_ids.addAll(ids);
					new OutputTable(ids, "Свод за " + month
						+ " " + year + "/"
						+ districts[p] + ".xls");
				    }
				}
				new OutputTable(full_ids, "Свод за " + month
					+ " " + year + "/Свод по КО.xls");
			...
			JMenuItem svod2 = new JMenuItem("Создать свод за "
				+ jDataTable.getValueAt(
					jDataTable.getSelectedRows()[0], 1));
			Pmenu.add(svod2);
			...
			svod2.addActionListener(new ActionListener() {
				...
				new OutputTableV2(year, "Свод за " + year
					+ ".xls");
			...
			JMenuItem svod = new JMenuItem("Создать свод из "
				+ jDataTable.getSelectedRows().length
				+ " выделенных");
			Pmenu.add(svod);
		    ...
		    JMenuItem delRecords = new JMenuItem("Удалить "
			    + jDataTable.getSelectedRows().length + " эл.");
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < jDataTable
					.getSelectedRows().length; i++) {
				    new ConnectionBD().deleteRow(jDataTable
					    .getValueAt(
						    jDataTable
							    .getSelectedRows()[i],
						    0).toString());
	...
	Vector values = new ConnectionBD().getDataFromDB(sqlsearch);
	...
	public void actionPerformed(ActionEvent e) {
		...
		ids.addElement(new Integer(jDataTable.getValueAt(
			jDataTable.getSelectedRows()[i], 0).toString()));
	    }
	    new OutputTable(ids, "свод.xls");
	}
    }
